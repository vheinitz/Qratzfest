...
//     VH: The URL is not active any mode (01.08.2014)
//=============================================================================
...
{    
    ...
    if ( !open(portName) )
    {
        PD_ERROR( "Cant open port \"%s\"", portName  );
    }
...
bool COMPort::open ( const char * portName )
{
    ...
    {
        PD_WARNING( "COMPort already uses port \"%s\"",portName  )
		close();
        //return false;
    }
					...
					if(devName.indexOf("COM", 0, Qt::CaseSensitive) == 0){
						if((devName.length() > 3) && (devName.length() < 6)){
							...
							if(portNr.toInt() > 0){
								...
								PD_TRACE(" Available Port: '%s' detected.", QS2CS(devName));
							}
						}
					}
